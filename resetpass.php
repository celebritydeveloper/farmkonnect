...
if(isset($_GET['id']) && isset($_GET['code']))
{
	...
	$stmt = $user->runQuery("SELECT * FROM farmkonnect_members WHERE userID=:uid AND tokenCode=:token");
	...
	if($stmt->rowCount() == 1)
	{
		if(isset($_POST['btn-reset-pass']))
		{
			...
			{
				$password = md5($cpass);
				$stmt = $user->runQuery("UPDATE farmkonnect_members SET userPass=:upass WHERE userID=:uid");
